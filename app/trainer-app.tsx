...
export default function TrainerApp({ initialContent }: { initialContent: TrainerContent }) {
  ...
  useEffect(() => { try { const saved = localStorage.getItem(`habra-progress-pages-v1:${location.pathname}`); if (saved) setProgress(JSON.parse(saved)); } catch { /* prázdný postup */ } finally { setProgressLoaded(true); } }, []);
  useEffect(() => { if (progressLoaded) localStorage.setItem(`habra-progress-pages-v1:${location.pathname}`, JSON.stringify(progress)); }, [progress, progressLoaded]);
